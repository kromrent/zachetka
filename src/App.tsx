...
function Lectures({ onBack }: { onBack: () => void }) {
  ...
  return <section className="lectures-page">
    ...
    {current && <LectureQuestionPractice key={current.id} track={current}/>}
...
function LectureQuestionPractice({ track }: { track: LectureTrack }) {
  ...
  return <div className="lecture-question-practice">
    {track.mode === "section" && questions.length > 1 && <label className="lecture-note-topic"><span>Материалы к вопросу</span><select value={activeIndex} onChange={(event) => setTopicIndex(Number(event.target.value))}>{questions.map((topic, index) => <option value={index} key={`${track.id}-practice-${index}`}>{index + 1}. {topic}</option>)}</select></label>}
    <QuestionNotes questionKey={questionKey} question={question} contextLabel={`${track.subject} · ${track.section}`}/>
    <QuestionMiniQuiz questionKey={questionKey} question={question} contextLabel={`${track.subject} · ${track.section}`}/>
  </div>;
}
...
function Exam({ subject, onBack }: { subject: Subject; onBack: () => void }) {
  ...
  if (result) return <section className="result">
    <button className="back" onClick={onBack}>← На главную</button>
    <p className="eyebrow">Экзамен завершён</p>
    <h1>{result.total}<span>/100</span></h1>
    <h2>{result.total >= 75 ? "Уверенный результат" : result.total >= 50 ? "Есть база — усилим слабые места" : "Нужна ещё тренировка"}</h2>
    <div className="ticket-scores"><span>Теория 1 <b>{result.theory[0].score}/25</b></span><span>Теория 2 <b>{result.theory[1].score}/25</b></span><span>Задача <b>{result.code.score}/50</b></span></div>
    <div className="exam-result-notes">
      <h3>Повторить вопросы билета</h3>
      {theory.map((question, index) => <article className="exam-result-topic" key={`${subject.id}:${question.id}`}>
        <p className="eyebrow">Вопрос {index + 1} · {question.section}</p>
        <h4>{question.text}</h4>
        <QuestionNotes questionKey={`${subject.id}:${question.id}`} question={question.text} contextLabel={`${bank.title} · ${question.section}`}/>
        <QuestionMiniQuiz questionKey={`${subject.id}:${question.id}`} question={question.text} contextLabel={`${bank.title} · ${question.section}`}/>
      </article>)}
    </div>
    <button className="primary" onClick={onBack}>На главную</button>
  </section>;
